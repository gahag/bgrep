...
fn grep_filename<O: Write, P: Display, B: AsRef<[u8]>>(
    ...
    if options.inverse {
        ...
        let inverse_match = matches.find(|m| {
        ...
        let matched = (inverse_match.is_some() || end < buffer.len()) ^ options.non_matching; // List non matching files.
...
fn grep_bytes<O: Write, P: Display, B: AsRef<[u8]>>(
    ...
    if options.inverse {
        ...
        if let Some(bs) = matches.next() {
            if !bs.is_empty() {
                // A regex may have a empty match, but when inverse matching
...
fn run_file<O: Write, P: AsRef<Path>, B: AsMut<Vec<u8>>>(
    ...
    let (read_result, path) = if path == Path::new(args::STDIN) {
        // Path::new is cost-free.
        (
            io::stdin().lock().read_to_end(buffer),
            Path::new("<stdin>").display(),
        )
    } else {
...
pub fn run<O: Write>(args: Args, out: &mut O) -> io::Result<bool> {
    ...
    let pattern = build_pattern(&pattern, &options).map_err(|e| {
